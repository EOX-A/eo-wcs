...
* For the _eowcs_geteocoverageset_ conformance class there are additional
`wcseo:packageFormatSupported` elements inside the `wcs:Extension` element of
the `wcs:ServiceMetadata` element to specify supported package formats for the
...
The response to a successful _GetCapabilities_ request of an EO-WCS supporting
the _eowcs_geteocoverageset_ conformance class *shall* list in
`wcseo:packageFormatSupported` elements inside the `wcs:Extension` element of
its `wcs:ServiceMetadata` the MIME type identifiers one to one of all supported
...
[requirement,uri="/req/eowcs_geteocoverageset/defaultPackageFormat"]
In the response to a successful _DescribeEOCoverageSet_ request, each
DatasetSeries listed *shall* contain in its `wcseo:defaultPackageFormat` element
inside its `wcseo:ServiceParameters` element the default format used for
_GetEOCoverageSet_ responses.
...
subsetted coverages in their native format with limited processing like
subsetting applied. To request custom formats or advanced processing the
...
[requirement,uri="/req/eowcs_geteocoverageset/getCoverage-acceptable-format"]
...
[requirement,uri="/req/eowcs_geteocoverageset/getCoverage-acceptable-mediaType"]

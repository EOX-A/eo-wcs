...
[<<epsg>>], the temporal constraint in ISO 8601 [<<iso8601>>].
...
A _DescribeEOCoverageSet_ request *shall* use WGS84 [<<epsg>>] as spatial and
ISO 8601 [<<iso8601>>] as temporal CRS for the coordinates in trim requests.
...
whereby all spatial coordinates are expressed in WGS84 [<<epsg>>].
...
- is completely contained within the request time extent, and the request
  parameter `containment` is of value `contains`, +
whereby all temporal coordinates are expressed in ISO 8601 [<<iso8601>>].

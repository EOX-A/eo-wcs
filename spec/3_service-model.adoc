...
coverages. Each coverage itself is structured the same way as resulting from a
_GetCoverage_ request.
...
TODO
  ...
  count
  dimensionTrim
  subsettingCrs in combination with dimensionTrim
...
The contents of the response to a successful _GetEOCoverageSet_ request *shall*
follow the structure of the `wcseo:EOCoverageSet` element if the `mediaType`
parameter is present and has the value `multipart/related`. The second part of
the multipart response *shall* be encoded as specified by
...
The coverages contained in the response to a successful _GetEOCoverageSet_
...
[requirement,uri="/req/eowcs_geteocoverageset/getEOCoverageSet-acceptable-scaling"]
If a _GetCoverage_ request contains a `Scaling` parameter then this parameter
*shall* be applied individually to each coverage to be returned as specified by
the WCS Scaling Extension [OGC 12-039].

[requirement,uri="/req/eowcs_geteocoverageset/getEOCoverageSet-acceptable-interpolation"]
If a _GetCoverage_ request contains a `Interpolation` parameter then this
parameter *shall* be applied individually to each coverage to be returned as
...
coverage to be returned as specified by the WCS CRS Extension [OGC 11-053r1].
